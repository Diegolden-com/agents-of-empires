...
const LLM_OPTIONS: Record<string, { label: string; models: { value: string; label: string; cost: string }[] }> = {
  openai: {
    label: 'OpenAI',
    models: [
      { value: 'gpt-4o', label: 'GPT-4o (Most Capable)', cost: '💰💰💰' },
      { value: 'gpt-4o-mini', label: 'GPT-4o Mini (Economic)', cost: '💰' },
      { value: 'gpt-4-turbo', label: 'GPT-4 Turbo', cost: '💰💰💰' },
      { value: 'gpt-3.5-turbo', label: 'GPT-3.5 Turbo (Very Economic)', cost: '💰' },
    ],
  },
  anthropic: {
    label: 'Anthropic (Claude)',
    models: [
      { value: 'claude-3-5-sonnet-20241022', label: 'Claude 3.5 Sonnet (Best)', cost: '💰💰💰💰' },
      { value: 'claude-3-5-haiku-20241022', label: 'Claude 3.5 Haiku (Fast)', cost: '💰' },
    ],
  },
  google: {
    label: 'Google (Gemini)',
    models: [
      ...
      { value: 'gemini-1.5-flash', label: 'Gemini 1.5 Flash (Very Fast)', cost: '💰' },
    ],
  },
  mistral: {
    label: 'Mistral',
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    {CATAN_AGENTS.map((agent) => (
                      <Card
                        key={agent.id}
                        className={`cursor-pointer transition-all ${selectedAgents.includes(agent.id)
                                    ...
                                    <div>
                                      <label className="text-xs font-semibold text-gray-700 mb-1 block">
                                        ...
                                        <span className="text-xs text-gray-500 ml-2">
                                          {currentTemp < 0.4 ? '(Very Conservative)' :
                                            currentTemp < 0.6 ? '(Conservative)' :
                                              currentTemp < 0.8 ? '(Balanced)' :
                                                currentTemp < 0.9 ? '(Creative)' : '(Very Risky)'}
                                        </span>
                                      </label>
                                      <input
                                        type="range"
              ...
              {gameId && (
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                  ...
                  <Card>
                    <CardHeader className="pb-3">
                      <div className="flex items-center justify-between">
                        <CardTitle>Catan Board</CardTitle>
                        <Badge variant="secondary">Turn {gameState?.turn || 0}</Badge>
                      </div>
                    </CardHeader>
                    <CardContent>
                      {gameState ? (
                      ...
                      <CardHeader className="pb-3">
                        <div className="flex items-center justify-between">
                          ...
                          >
                            ...
                            {showEventDetails ? 'Compact' : 'Detailed'}
                          </Button>
                        </div>
                      </CardHeader>
...
function CompactEventCard({ event, showDetails }: { event: any; showDetails: boolean }) {
  ...
  if (event.type === 'thinking') {
    return (
      <div className="p-2 bg-gray-50 border-l-4 border-gray-300 rounded text-sm">
        <div className="text-gray-600">🤔 {event.playerName} thinking...</div>
      </div>
    );
  }
  ...
  if (event.type === 'action_result') {
    return (
      <div className={`p-2 border-l-4 rounded text-sm ${event.success
    ...
    return (
      <div className="p-2 bg-blue-100 border-l-4 border-blue-600 rounded text-sm">
        <div className="font-semibold text-blue-900">
          ⏱️ Turn {event.turn} - {event.currentPlayer.name}
        </div>
      </div>
    );
  }
