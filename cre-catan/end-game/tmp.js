...
var configSchema = exports_external.object({
  ...
  consumerAddress: exports_external.string(),
  gasLimit: exports_external.string(),
  gameId: exports_external.string(),
  winner: exports_external.number()
});
var endGameOnchain = (runtime2) => {
  runtime2.log("=".repeat(50));
  runtime2.log("End Game Workflow");
  runtime2.log("=".repeat(50));
  ...
  const reportData = encodeAbiParameters(parseAbiParameters("uint256 gameId, uint8 winner"), [BigInt(runtime2.config.gameId), runtime2.config.winner]);
  runtime2.log(`Encoded data for Game ${runtime2.config.gameId}, Winner: ${runtime2.config.winner}`);
  ...
  runtime2.log(`Generated signed report`);
  const writeResult = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.consumerAddress,
  ...
  if (writeResult.txStatus === TxStatus.SUCCESS) {
    ...
    runtime2.log(`Game ${runtime2.config.gameId} ended. Winner: Player ${runtime2.config.winner}`);
    runtime2.log("=".repeat(50));
    return txHash;
  }
