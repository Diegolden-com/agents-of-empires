...
var readGameFromBlockchain = async (runtime2, gameId) => {
  try {
    runtime2.log("*********************");
    runtime2.log("Reading game from blockchain");
    runtime2.log(`Game ID: ${gameId}`);
    runtime2.log("*********************");
...
var onLogTrigger = async (runtime2, log) => {
  ...
  try {
    ...
    if (decodedLog.eventName === "GameActivated") {
      ...
      const apiUrl = "http://localhost:3000/api/game/start";
      const result = httpClient.sendRequest(runtime2, httpRequest(gamePayload, apiUrl), consensusIdenticalAggregation())(runtime2.config).result();
      runtime2.log(`✓ Game ${gamePayload.gameId} data sent to API with status: ${result.statusCode}`);
      return `Game ${gameId.toString()} activated successfully`;
    }
  } catch (error) {
    runtime2.log(`Error decoding log: ${error}`);
...
var initWorkflow = (config) => {
  ...
  console.log(`Network found: ${network248.chainSelector.name}`);
  const evmClient = new cre.capabilities.EVMClient(network248.chainSelector.selector);
  return [
    cre.handler(cron.trigger({ schedule: config.schedule }), onCronTrigger),
    cre.handler(evmClient.logTrigger({ addresses: [config.gameControllerAddress] }), onLogTrigger)
  ];
};
async function main() {
